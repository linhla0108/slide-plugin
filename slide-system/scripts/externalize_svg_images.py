...
For every `artifact/visual.svg`, `artifact/source-page.svg`, and
`evidence/source-with-text.svg` in the batch it lifts base64 data-image payloads
into `artifact/assets/`, deduplicates by content hash, rewrites the SVG to
reference the files relatively, and records the result in
`evidence/external-images.json` plus the item `mapping.json` and
`evidence/notes.md`.
"""
...
def main() -> int:
    ...
    print(f"Externalized SVG images across {count} SVG file(s)")
    return 0
