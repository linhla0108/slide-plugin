---
name: make-a-deck
description: Create a self-contained HTML presentation deck with strong narrative structure, projection-ready typography, editable static slide content, speaker notes, and professional slide design. Use when the user asks to create or design a presentation deck.
---
...
Build at 1920×1080 (16:9). Do NOT hand-roll the stage/scaling/nav scaffolding — copy `slide-system/boilerplates/deck_stage.js` to the run as `deck_stage.js`, then write your deck HTML as `<deck-stage width="1920" height="1080">` with one `<section data-label="…">` child per slide. The component handles letterboxed scaling, keyboard navigation, active-slide state, and the `noscale` capture mode. Load it with a plain `<script src="deck_stage.js"></script>` — it is vanilla JS, not JSX. (For PPTX export later: pass `resetTransformSelector: "deck-stage"` to gen_pptx — the component honours a `noscale` attribute so capture sees authored-size geometry.)
